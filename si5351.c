 ...
 * Ported to C & support for ST HAL by Simon Kennedy, G0FCU, 2018.
...
//Si5351IntStatus dev_int_status = {.SYS_INIT_STKY = 0, .LOL_B_STKY = 0,.LOL_A_STKY = 0, .LOS_STKY = 0};
...
Si5351_Si5351(uint8_t i2c_addr)
...
bool Si5351_init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
        ...
        do
        {
            status_reg = Si5351_si5351_read(SI5351_DEVICE_STATUS);
        } while (status_reg >> 7 == 1);

        // Set crystal load capacitance
        status_reg = Si5351_si5351_write(SI5351_CRYSTAL_LOAD, (xtal_load_c & SI5351_CRYSTAL_LOAD_MASK) | 0b00010010);
...
void Si5351_reset(void)
{
    ...
    // First, turn them off
    Si5351_si5351_write(16, 0x80);
    Si5351_si5351_write(17, 0x80);
    Si5351_si5351_write(18, 0x80);
    Si5351_si5351_write(19, 0x80);
    Si5351_si5351_write(20, 0x80);
    Si5351_si5351_write(21, 0x80);
    Si5351_si5351_write(22, 0x80);
    Si5351_si5351_write(23, 0x80);

    // Turn the clocks back on...
    Si5351_si5351_write(16, 0x0c);
    Si5351_si5351_write(17, 0x0c);
    Si5351_si5351_write(18, 0x0c);
    Si5351_si5351_write(19, 0x0c);
    Si5351_si5351_write(20, 0x0c);
    Si5351_si5351_write(21, 0x0c);
    Si5351_si5351_write(22, 0x0c);
    Si5351_si5351_write(23, 0x0c);
    ...
    Si5351_si5351_write(SI5351_VXCO_PARAMETERS_LOW, 0);
    Si5351_si5351_write(SI5351_VXCO_PARAMETERS_MID, 0);
    Si5351_si5351_write(SI5351_VXCO_PARAMETERS_HIGH, 0);
 ...
 */
//*********
//*****this version of set_freq is for use with Tayloe detector where 90 deg phase shift is required
//*********
uint8_t Si5351_set_freq(uint64_t freq, enum si5351_clock clk, uint16_t phshift)
{
    ...
    uint8_t div_by_4 = 0;
    uint8_t r_div = 0;
    extern uint16_t old_phshift;
    extern uint32_t a;
    int64_t orig_pll_freq, new_pll_freq;
    extern int64_t old_pll_freq;
    extern uint16_t orig_ms_int, ms_int;
    int8_t direction = 0;
    bool ms_int_changed = siFALSE;
    ...
    if(clk_first_set[(uint8_t)clk] == siFALSE)
   	{
        //first time through only set up
        Si5351_output_enable(clk, 1);
        Si5351_output_enable(clk + 2, 1);
        old_phshift = 99;
        clk_first_set[(uint8_t)clk] = siTRUE;
        // Select the proper R div value
        r_div = Si5351_select_r_div(&freq);
        new_pll_freq = 0;
        ms_int_changed = siTRUE;
   	}
    new_pll_freq = freq * ms_int;
    if(new_pll_freq < SI5351_PLL_VCO_MIN * SI5351_FREQ_MULT || new_pll_freq > SI5351_PLL_VCO_MAX * SI5351_FREQ_MULT)
    {
        direction = 1;
        ms_int = 62;
        ms_int_changed = siFALSE;

        while(new_pll_freq < SI5351_PLL_VCO_MIN * SI5351_FREQ_MULT || new_pll_freq > SI5351_PLL_VCO_MAX * SI5351_FREQ_MULT)
        	{
        	ms_int = ms_int + (2 * direction);
        	new_pll_freq = freq * ms_int;
        	if(new_pll_freq < SI5351_PLL_VCO_MIN * SI5351_FREQ_MULT && new_pll_freq - old_pll_freq < 0)
        		{
        		direction = direction * -1;
        		}
        	if(new_pll_freq > SI5351_PLL_VCO_MAX * SI5351_FREQ_MULT && new_pll_freq - old_pll_freq > 0)
        		{
        		direction = direction * -1;
        		}
        	old_pll_freq = new_pll_freq;
        	ms_int_changed = siTRUE;
        	//sprintf(x, "ms %i ps %i pll %lu", ms_int, phshift, new_pll_freq);
        	//gwinSetText(version, x, TRUE);
        	}
     }

     Si5351_set_pll(new_pll_freq, pll_assignment[clk]);
     Si5351_set_pll(new_pll_freq, pll_assignment[clk + 2]);
     //set new MS div
     if(ms_int_changed)
     {
        div_by_4 = 0;
        int_mode = 1;
        ms_reg.p1 = 128 * ms_int + ((128 * 0) / 1) - 512;
        ms_reg.p2 = 0;
        ms_reg.p3 = 1;

        Si5351_set_ms(clk, ms_reg, int_mode, r_div, div_by_4);
        Si5351_set_ms(clk + 2, ms_reg, int_mode, r_div, div_by_4);
        //Si5351_set_ms(SI5351_CLK0, ms_reg, int_mode, r_div, div_by_4);
        //if(clk == SI5351_CLK2)
        //{
        	Si5351_set_phase(clk + 2, (uint8_t)ms_int);
        	sprintf(x, "clk %i ms %i rd %i ps %i pll %l", clk+2, ms_int, r_div, phshift, new_pll_freq);
        	gwinSetText(version, x, TRUE);
        //}
        //else
        //{
        	Si5351_set_phase(clk, 0);
        	//sprintf(x, "clk %i ms %i rd %i ps %i pll %l", clk, ms_int, r_div, phshift, new_pll_freq);
        	//gwinSetText(version, x, TRUE);
        //}
        Si5351_pll_reset(pll_assignment[clk]);
        Si5351_pll_reset(pll_assignment[clk + 2]);
    	//sprintf(x, "clk %i ms %i rd %i ps %i pll %l", clk, ms_int, r_div, phshift, new_pll_freq);
    	//gwinSetText(version, x, TRUE);
     }

    //***SHK***write appropriate phase offset register for the clock that you want 90 deg phase shift on, CLK0 is reg 165
   /*if(phshift != old_phshift)
    	{
    	 //sprintf(x, "ph %i, %i", old_phshift, phshift);
    	 //gwinSetText(version, x, TRUE);

        old_phshift = phshift;
        if(phshift == 90 || phshift == 270)
        	{
        	Si5351_set_phase(clk, ms_int);//ms_int
            if(phshift == 270)
            	{
            	//set 'invert' bit
            	}
            // Reset the PLL ***only when multisynth dividers or phase shift settings are changed***SHK
            Si5351_pll_reset(pll_assignment[clk]);
...
void Si5351_set_pll(uint64_t pll_freq, enum si5351_pll target_pll)
{
  ...
  if(target_pll == SI5351_PLLA)
  {
	  Si5351_si5351_write_bulk(SI5351_PLLA_PARAMETERS, i, params);
        plla_freq = pll_freq;
  }
  else if(target_pll == SI5351_PLLB)
  {
	  Si5351_si5351_write_bulk(SI5351_PLLB_PARAMETERS, i, params);
        pllb_freq = pll_freq;
  }
...
void Si5351_set_ms(enum si5351_clock clk, struct Si5351RegSet ms_reg, uint8_t int_mode, uint8_t r_div, uint8_t div_by_4)
{
    ...
    if((uint8_t)clk <= (uint8_t)SI5351_CLK5)
    {
        ...
        reg_val = Si5351_si5351_read((SI5351_CLK0_PARAMETERS + 2) + (clk * 8));
        reg_val &= ~(0x03);
    ...
    {
        case SI5351_CLK0:
        	Si5351_si5351_write_bulk(SI5351_CLK0_PARAMETERS, i, params);
        	Si5351_set_int(clk, int_mode);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
        case SI5351_CLK1:
        	Si5351_si5351_write_bulk(SI5351_CLK1_PARAMETERS, i, params);
        	Si5351_set_int(clk, int_mode);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
        case SI5351_CLK2:
        	Si5351_si5351_write_bulk(SI5351_CLK2_PARAMETERS, i, params);
        	Si5351_set_int(clk, int_mode);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
        case SI5351_CLK3:
        	Si5351_si5351_write_bulk(SI5351_CLK3_PARAMETERS, i, params);
        	Si5351_set_int(clk, int_mode);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
        case SI5351_CLK4:
        	Si5351_si5351_write_bulk(SI5351_CLK4_PARAMETERS, i, params);
        	Si5351_set_int(clk, int_mode);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
        case SI5351_CLK5:
        	Si5351_si5351_write_bulk(SI5351_CLK5_PARAMETERS, i, params);
        	Si5351_set_int(clk, int_mode);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
        case SI5351_CLK6:
        	Si5351_si5351_write(SI5351_CLK6_PARAMETERS, temp);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
        case SI5351_CLK7:
        	Si5351_si5351_write(SI5351_CLK7_PARAMETERS, temp);
        	Si5351_ms_div(clk, r_div, div_by_4);
            break;
    }
...
void Si5351_output_enable(enum si5351_clock clk, uint8_t enable)
{
  ...
  reg_val = Si5351_si5351_read(SI5351_OUTPUT_ENABLE_CTRL);
  ...
  Si5351_si5351_write(SI5351_OUTPUT_ENABLE_CTRL, reg_val);
}
...
void Si5351_drive_strength(enum si5351_clock clk, enum si5351_drive drive)
{
  ...
  reg_val = Si5351_si5351_read(SI5351_CLK0_CTRL + (uint8_t)clk);
  reg_val &= ~(mask);
  ...
  Si5351_si5351_write(SI5351_CLK0_CTRL + (uint8_t)clk, reg_val);
}
...
void Si5351_set_phase(enum si5351_clock clk, uint8_t phase)
{
    ...
    Si5351_si5351_write(SI5351_CLK0_PHASE_OFFSET + (uint8_t)clk, phase);
}
...
void Si5351_pll_reset(enum si5351_pll target_pll)
{
    if(target_pll == SI5351_PLLA)
    {
    	Si5351_si5351_write(SI5351_PLL_RESET, SI5351_PLL_RESET_A);
    }
    else if(target_pll == SI5351_PLLB)
    {
    	Si5351_si5351_write(SI5351_PLL_RESET, SI5351_PLL_RESET_B);
    }
}
...
void Si5351_set_clock_disable(enum si5351_clock clk, enum si5351_clock_disable dis_state)
{
    ...
    reg_val = Si5351_si5351_read(reg);
    ...
    Si5351_si5351_write(reg, reg_val);
}
...
{
    uint8_t reg_val;
    reg_val = Si5351_si5351_read(SI5351_FANOUT_ENABLE);
    ...
    Si5351_si5351_write(SI5351_FANOUT_ENABLE, reg_val);
}
...
{
    uint8_t reg_val;
    reg_val = Si5351_si5351_read(SI5351_PLL_INPUT_SOURCE);
    ...
    Si5351_si5351_write(SI5351_PLL_INPUT_SOURCE, reg_val);
...
void Si5351_set_vcxo(uint64_t pll_freq, uint8_t ppm)
{
    ...
    Si5351_si5351_write(SI5351_VXCO_PARAMETERS_LOW, temp);

    temp = (uint8_t)((vcxo_param >> 8) & 0xFF);
    Si5351_si5351_write(SI5351_VXCO_PARAMETERS_MID, temp);

    temp = (uint8_t)((vcxo_param >> 16) & 0x3F);
    Si5351_si5351_write(SI5351_VXCO_PARAMETERS_HIGH, temp);
}
...
uint8_t Si5351_si5351_write_bulk(uint8_t reg, uint8_t bytes, uint8_t *data)
{
  ...
  for (uint16_t i = 0; i < bytes; i++)
    {
      data_a[i+1] = data[i];
      data_b = data[i];
      while (HAL_I2C_IsDeviceReady(&I2cHandle, (uint16_t)(I2C_ADDRESS<<1), 3, I2C_TIMEOUT) != HAL_OK) { }

      while( HAL_I2C_Mem_Write(&I2cHandle,							// i2c handle
    	    				(uint8_t)(I2C_ADDRESS<<1),		// i2c address, left aligned
    						(uint8_t)reg+i,						// register address
    						I2C_MEMADD_SIZE_8BIT,				// si5351 uses 8bit register addresses
    						(uint8_t*)(&data_b),				// write returned data to this variable
...
uint8_t Si5351_si5351_write(uint8_t reg, uint8_t data)
{
    char data_a[2];
    uint8_t data_b;
    data_a[0] = reg;
    data_a[1] = data;
    data_b = data;
    //return i2c.write(i2c_bus_addr,data_a,2);
	HAL_StatusTypeDef status = HAL_OK;
...
uint8_t Si5351_si5351_read(uint8_t reg)
{
    char data_a[1] = {reg};
    uint8_t data_read = 0;
    //i2c.write(i2c_bus_addr, data_a, 1);
    //i2c.read(i2c_bus_addr,data_read,1);
	HAL_StatusTypeDef status = HAL_OK;

	while (HAL_I2C_IsDeviceReady(&I2cHandle, (uint16_t)(I2C_ADDRESS<<1), 3, I2C_TIMEOUT) != HAL_OK) { }

	while(HAL_I2C_Mem_Read(&I2cHandle,						// i2c handle
    						  (uint8_t)(I2C_ADDRESS<<1),		// i2c address, left aligned
							  (uint8_t)reg,						// register address
							  I2C_MEMADD_SIZE_8BIT,				// si5351 uses 8bit register addresses
							  //(uint8_t*)(&value),				// write returned data to this variable
							  (uint8_t*)(&data_read),				// write returned data to this variable
							  1,								// how many bytes to expect returned
							  I2C_TIMEOUT)						// timeout
	!= HAL_OK) { ErrorHandler(); }
  //return ERROR_NONE;

  return (uint8_t) data_read;//reg_val;
}
...
{
    uint64_t lltmp, max_vco;
    uint32_t b, c, p1, p2, p3;
    extern uint32_t a;
    uint8_t divby4 = 0;
    uint8_t ret_val = 0;
...
void Si5351_ms_div(enum si5351_clock clk, uint8_t r_div, uint8_t div_by_4)
{
    ...
    reg_val = Si5351_si5351_read(reg_addr);
    ...
    Si5351_si5351_write(reg_addr, reg_val);
}
